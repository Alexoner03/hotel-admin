<template>
  <q-card style="width: 700px; max-width: 80vw;" class="q-py-md">
    <q-card-section>
      <div class="text-h6">
        {{ formData ? "Editar Cliente" : "Registrar Cliente" }}
      </div>
    </q-card-section>
    ...
    <q-card-section class="q-px-xl q-py-sm">
      <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-sm">
        ...
        <q-input
          ...
          type="text"
          label="RUC"
          hint="Ejemplo : 10838763821"
          :rules="[
            val => !!val || 'Este Campo es Requerido',
            val => val.length == 11 || 'Deben ser 11 digitos'
          ]"
        />
        <q-input
  ...
  methods: {
    ...
    setFormData() {
      if (this.formData) {
        ...
        this.ruc = this.formData.ruc;
        this.business_name = this.formData.business_name;
      }
    },
    async create(body) {
      const { url, headers } = links.createClient;
      try {
    ...
    async edit(body) {
      const { url, headers } = links.editClientUrl(this.formData.id);
      try {
